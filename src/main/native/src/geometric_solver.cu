...
__device__ graph::Geometric *EvaluationContext::getGeomtricObject(int geometricId) const { return const_cast<graph::Geometric *>(&geometrics[geometricId]); };
...
/// @return void
///
template <size_t TZ> __device__ void SetupPointAccessor(EvaluationContext *ec, int N, /* __shared__*/ double _points[TZ]) {
        int offset;
