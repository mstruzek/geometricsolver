    ...
    bool structuralZero)
{
    ...
    base = 0;
    if (base1) {
            // INVERSE rewrite coo into base = 0
            for (i = 0; i < (*nnz); i++) {
                    tempRowInd[i] = tempRowInd[i] - 1;
                    tempColInd[i] = tempColInd[i] - 1;
            }
            base1 = 0;
            base0 = 0;            
    }
    ...
    int N = std::max(*m, *n);
/// list all diagonal elements 1 or 0 if missing structural value
    std::vector<int> diagonalSet = std::vector<int>(N); 
...
// use qsort to sort COO format 
    ...
    for(i=0; i<(*nnz); i++){
        if (tempRowInd[i] == tempColInd[i]) diagonalSet[tempColInd[i]] = 1;
        work[i].i = tempRowInd[i];
        work[i].j = tempColInd[i];
        work[i].p = i; // permutation is identity
    }

    /// update structural zero on diagonal if not set
    if (structuralZero) {
        int diag_count = std::reduce(std::begin(diagonalSet), std::end(diagonalSet), 0,  std::plus<int>());
        int allocSize = N - diag_count;
        if (allocSize) {
            /// update COO tempRowInd
            int *tempRowIndTmp = (int *)malloc(sizeof(int) * (*nnz + allocSize));
            memcpy(tempRowIndTmp, tempRowInd, sizeof(int) * (*nnz));
            free(tempRowInd);
            tempRowInd = tempRowIndTmp;
            
            /// update COO tempColInd
            int *tempColIndTmp = (int *)malloc(sizeof(int) * (*nnz + allocSize));
            memcpy(tempColIndTmp, tempColInd, sizeof(int) * (*nnz));
            free(tempColInd);
            tempColInd = tempColIndTmp;

            /// update COO values
            double *tempValTmp = (double *)malloc(sizeof(double) * (*nnz + allocSize));
            memcpy(tempValTmp, tempVal, sizeof(double) * (*nnz));
            free(tempVal);
            tempVal = tempValTmp;

            /// update COO workTmp
            cooFormat *workTmp = (struct cooFormat *)malloc(sizeof(struct cooFormat) * (*nnz + allocSize));
            memcpy(workTmp, work, sizeof(struct cooFormat) * (*nnz));
            ...
            int i = *nnz;
            int index = 0;
            for (auto itr = diagonalSet.begin(), end = diagonalSet.end(); itr != end; ++index, ++itr) {
                int value = *itr;
                if (!value) {
                    work[i].i = index;
                    work[i].j = index;
